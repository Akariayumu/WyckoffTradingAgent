...
def test_weekly_reflection_gate_uses_worker_dispatch_input():
    """Worker 根据计划时间传入跳过标志，不能根据 Actions 实际起跑日期判断。"""
    workflow = Path(".github/workflows/signal_feedback.yml").read_text(encoding="utf-8")

    assert "$(date -u +%u)" not in workflow
    assert "SKIP_WEEKLY_REFLECTION: ${{ inputs.skip_weekly_reflection }}" in workflow
    assert '[ "${SKIP_WEEKLY_REFLECTION}" = "true" ]' in workflow
