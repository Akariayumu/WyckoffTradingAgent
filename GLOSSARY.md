...
工作日 12:00 运行的全 A 股只读研究任务，与盘后 Wyckoff 漏斗相互独立。一次运行内按“有效横截面 → 可成交池 → 六通道实时注意力池 → 深度证据池 → 六视角60只短名单 → 10—20只公告终审”逐层收敛，并记录每层数量、淘汰原因和真实耗时。六个研究视角为质量成长、合理估值、盈利变化、稳定相对强度、行业扩散和逆向修复；任何单一技术形态或综合分都不能直接生成买入结论。

独立午间研究以本次直接查询的线上 `USER_LIVE:<user_id>` 原表值为准。持仓、股数、成本、止损、现金及 state signature 的正常变化，以及 `updated_at` 未同步、滞后或缺失，仅作非阻断审计提示，不要求与历史一致，也不因此中止研究。必要时复查一次并采用最后成功读取的当前值。只有身份/项目不匹配、查询失败、组合缺失或必需数值不可用时，局部停用受影响的持仓动作计算；独立市场与消息研究继续。此规则不授权修改账户或放宽行情、原文尽调门槛。
...
#### 三强核心形态（当前生产默认启用）
全市场 5,476 只股票 6 年实测确立的三强核心形态组合，扣除 0.202% 往返摩擦后净均值达 **+0.745%** ($t=13.64$)：
- **Spring（终极震仓）**：吸筹末期，股价突然跌破支撑位引发恐慌卖盘，随后快速拉回。正式检测优先使用近期 swing low 中位数作为抗噪支撑，摆动点不足时回退窗口最低收盘价。
- **EVR (Effort vs Result / 放量抗跌)**：成交量巨幅放大（巨大 Effort），但股价没有相应大跌（Result 不差），意味着主力在底部大量承接。
- **Trend Pullback（趋势回踩）**：强趋势股（均线多头）在回踩关键均线缩量企稳时介入，捕捉主升趋势的二次上车机会。

#### 已废弃负超额形态（生产默认关闭）
以下形态在全市场 6 年实证中确认产生负超额或严重失血，已在生产配置中默认关闭（`False`）：
- **Compression（压缩蓄势）**：实测净均值 **-0.170%**，多空平衡被打破后往往向下击穿；已于 PR #458 关闭。
- **SOS (Sign of Strength)**：放量突破，实测净均值 **-0.061%**，在 A 股追逐放量大阳线容易追高吃面；已于 PR #458 关闭。
- **LPS (Last Point of Support)**：各变体全周期净均值均为负（原版 **-0.159%**，Fib 动态放宽版 **-1.319%**）；已于 PR #460 关闭。
...
| **推荐池累计涨幅** | `/tracking` 组头/摘要的「首次至今」由窗口内该股最早事件价推算；主表每行 `change_pct` 只相对该行入选日收盘。不能把首次至今涨幅归因到每一次重复入选或次日买入 |
| **收益覆盖率** | 同时报请求候选、双端有效价格观测、缺价和低样本日剔除数量；没有有效基准的日期不参与“跑赢指数”结论 |
...
| **主线引擎** | Mainline Engine | L1 后并联的主题候选源，基于概念热度、概念映射、主题雷达、财务质量和量价 timing 发现 A 股主线票；全部有效研究条目参与合并，最终 AI cap 不变 | 不提前截 Top3 |
| **候选车道** | Candidate Lane | L1 通过但传统 L2/L4 尚未完全成形的趋势回踩、强承接、平台突破观察样本 | 观察池 |
| **L3** | 行业共振 | 生产全面采用 **Top 5 纯行业单标签共振**（`top_n_sectors: 5`, `use_concept_map: False`），扣费后净均值达 +0.235% ($t=15.57$)，大幅优于旧版多标签概念模式 | ~50% |
| **L4** | Wyckoff 触发 | 生产默认仅启用**三强核心形态**（`Spring` + `EVR` + `TrendPullback`），全市场净均值达 +0.745% ($t=13.64$)；已废弃负超额形态（SOS/LPS/Compression）默认关闭 | ~90% |
...
| **L5** | 退出信号 | 持有期间监控是否出现派发信号（SOW / UTAD）或触发 ATR 动态止损（`max(成本 - 2*ATR, 成本 * 0.90)`），决定是否提前退出。持仓诊断只喂入建仓日之后的价格路径；`buy_dt` 缺失或切不出建仓后 K 线时 fail-closed，不发结构退出，建仓前的高点与暴跌不参与破位判定。新多头 `add` 必须写入合法建仓日；`update` 与成交回填都不得在空账本新建无日期持仓 | — |
...
| **新仓止损配置** | 当前生产 `STEP4_BUY_HARD_STOP_PCT=5.0`；12% 是代码默认而非线上值。结构候选风险护栏与实际工单止损分层，已有持仓继承已持久化止损 |
...
| **research_discovery** | 当次生产 trace、候选、主线和趋势雷达的研究并集，保留发现/信号/拦截/待确认条件；禁买日也展示，不写成推荐或已获 OMS 许可 |
| **persisted_fallback** | 缺少同日有效主题雷达时使用的历史降级快照；必须有合法非未来日期且未超过 TTL，不能覆盖同日有效结果 |
...
| 状态 | 英文 | 含义 | 新开仓策略 |
|------|------|------|---------|
| **中性** | NEUTRAL | 中性水温，研究与交易许可分开 | 当前生产加入禁买名单，**禁止新开仓**；研究池仍可见 |
...
|------|--------------------------|------|
| NEUTRAL | **+1.17%** | 历史片段数字，不足以推导当前可交易或净超额为正 |
| RISK_ON | **-1.54%** | 过热追高亏钱 → **正式禁止新开仓** |
| CRASH | **-3.2%** | 历史片段数字；当前禁止新开仓 |

上述旧片段未在此表声明费用和完整分母，不可作为增益证明。生产交易许可读取市场闸门；研究发现、AI 复核、跨日确认与 OMS 审批必须分别报告。完整验收约束见 [研究与执行验收](docs/A_SHARE_RESEARCH_EXECUTION_ACCEPTANCE.md)。
...
| **开盘现金时序** | Open-before-exit Cash Timing | 验收比较器的显式保守模式：开盘先按有效开盘价配仓，再处理当日未明确时刻的退出，最后收盘估值；不借用未来卖出资金、仓位或收盘价格。旧入口的兼容时序不等于此模式，缺开盘不得填收盘后宣称完整 |
| **涨跌停成交约束** | Limit-move Fill Constraint | 回测按板块涨跌幅判定能否成交：开盘即封涨停（含盘中被砸开的 T 字板）买不到，一字跌停日卖不掉，到期平仓顺延至第一个可成交日 |

**因此**：回测可用于受限条件下的方向研究；是否足以支持参数调整，仍须核对历史时点、费用、分母和跨阶段一致性。样本内净收益提高不等于回撤改善，也不能承诺实盘绝对收益。
...
| **标的池行情查询** | TickFlow GET `/v1/quotes?universes=CN_Equity_A`，全A一次取数后本地筛选；不等于逐股查询，也不等于完成尽调。补充代码用逗号分隔的 `symbols` 参数；历史日K默认每批200只（美股通道在 workflow 层压到100），分钟接口上限独立。 |
...
| **推荐价 / initial_price** | 该行入选日的收盘价（事件价）。同一股票跨日再入选会新增一行，不再粘住首次价。`change_pct` 相对该事件价；MFE/MAE 仍按该行事件日独立计算。股票级「首次至今」只在 `/tracking` 组头/摘要里由最早事件价推算。 |
