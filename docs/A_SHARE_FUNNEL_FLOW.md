...
漏斗 Step2/3 成功后，`workflows/shadow_ledger_job.py` 另跑一套纸面账本：买许可与 Step4 规则准入相同，但只写 `shadow_*`。盘后定计划、次日官方开盘价成交，遵守 T+1 / 整手 / 涨跌停 / 费用；失败只记日志，不阻断漏斗。独立飞书卡标题为「影子账本 / paper」，不是实盘，也不是 `ic_shadow`。持仓区只列仍开着的仓，逐只展示盯市净收益（市值 − 含买费成本）及百分比；已卖光的名字不出现。账户汇总的现金 / 净值 / 市值 / 累计不变。
...
一致时才允许复用，避免把真正改变选股或信号的实验误当成轻量仓位对照。改变台账的变体各占一个独立分片：
I（确认分校准）与 J（仅开 `enable_two_track_mode` 的 Layer 2 纯两轨制，#476）都单独重跑全市场漏斗，报表按
`backtest-strategy-<period>-<variant>/` 目录名归组后与同窗口的 A 对照。
