...
本节仅描述行情传输，不把生产策略信号引入独立午间研究。

- 实时报价使用 GET `/v1/quotes`，`universes=CN_Equity_A` 整池取数后本地筛选，不逐股拉全A。`symbols` 与 `universes` 均为逗号分隔查询参数。厂商同时接受 POST JSON（2026-09-04 实测两种写法返回字段一致），统一走 GET 是为了跟客户端其余批量接口同源，不是因为 POST 不可用。
- 指数及市场池未覆盖的持仓各合并批量查询；同一次研究复用已核验的上午快照，不能跨日复用或混入午后价格。
- 历史日K使用 `get_klines_batch`，默认每批200只；美股通道在 `wyckoff_funnel_us.yml` 里单独压到100，那是该通道的选择，不是厂商限制。上午分钟线使用独立的 `/v1/klines/intraday/batch`，按该接口自己的限制分块。
- 批量失败必须披露请求数、错误、成功/缺失覆盖；禁止对整个市场逐股兜底。传输完成不等于原文尽调完成，未完成不得输出可执行换仓。
...
候选采用质量池：形态达标者共同排序，最终最多 **8 只**、单行业最多 **2 只**；这不是交易许可。当前生产 NEUTRAL 加入禁买名单。
...
| 水温 / 模式 | 新开仓 | 你怎么做 |
|-------------|--------|----------|
| **NEUTRAL**（生产 `execution_blocked`） | **禁止** | 观察主线与结构，不从研究池下单 |
...
2. 看完整 **主线买点研究池**及发现/待确认/拦截原因；不再误把前三只当完整发现范围，最终送审额度不变
...
- 非主线满 **5 日**进入时间管理；止损以实际工单及持仓已保存的止损为准
...
北京时间 16:05 的净值快照会把**当日盈亏**写入 `daily_nav.day_pnl`（账本合计）和
`position_day_pnl`（当前持仓逐只金额与涨跌幅）。口径是相对昨收（不是相对成本）；
`buy_dt` 会被当日加仓刷新成今天，不能当整仓今开，故有昨收时不改用成交价，仅昨收缺失
时才用今开成本兜底。现金记 0。随后 Step4 Telegram 工单顶部回读同一行，标题「当日盈亏」。
飞书漏斗卡和影子账本卡不承担实盘账户盈亏；Web mockup 里的「当日盈亏」KPI 仍是设计稿。
